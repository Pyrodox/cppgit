...
using std::string; using std::iterator; using std::vector; using std::find; using std::ostream_iterator;
using std::istream; using std::sort; using std::pair; using std::copy; using std::back_inserter; using std::list;
...
int main()
{
    ...
    //tosort pairstosort;
    map<string, list<int> > lst;
    for (auto it = ret.begin(); it != ret.end(); ++it) {
        for (int i = 0; i < it->second.size(); ++i) {
            lst[it->first].push_back(it->second[i]);
        }
    }
